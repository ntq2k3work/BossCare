        ...
        <div className="grid gap-6">
          <header className="grid gap-5 pt-1">
            <nav className="flex items-center gap-3 text-sm font-semibold text-slate-500">
              <Link href={`/dashboard/pets/${pet.id}`} className="text-slate-700 hover:text-violet-700">
                {copy.common.back}
              </Link>
              <Link href={`/dashboard/pets/${pet.id}/vaccinations`} className="hover:text-violet-700">
                {copy.petDetail.upcomingVaccinationsTitle}
              </Link>
              <span>›</span>
              <span>{copy.petDetail.addPet}</span>
            </nav>
            <div className="flex items-center gap-5">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight text-slate-950 md:text-4xl">{copy.vaccinations.addDoseTitle}</h1>
                <p className="mt-3 text-base text-slate-500">{copy.vaccinations.description}</p>
              </div>
            </div>
          </header>
